(() => {
    ...
    function initSliders() {
        if (document.querySelector(".info-block__slider")) new Swiper(".info-block__slider", {
        ...
        resizableSwiper("(max-width: 991.98px)", ".tariffs__slider", {
            modules: [],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
        ...
        if (partnersSlider) partnersSlider.forEach((parent => {
            const swiperNextBtn = parent.parentElement.querySelector(".swiper-button-next");
            const swiperPrevBtn = parent.parentElement.querySelector(".swiper-button-prev");
    ...
    const videoBlock = document.querySelector(".hero__video");
    const videoPLay = videoBlock.querySelector(".hero__video-play");
    const videoBlockVideo = videoBlock.querySelector(".hero__video video");
    if (videoBlock) {
        videoPLay.addEventListener("click", (() => {
            videoBlock.classList.add("video-play");
            if (videoBlockVideo) videoBlockVideo.play();
        }));
        videoBlockVideo.addEventListener("click", (() => {
            videoBlock.classList.remove("video-play");
            if (videoBlockVideo) videoBlockVideo.pause();
        }));
    }
    function animateNumber(element, start, end, duration) {
        ...
        const animate = () => {
            ...
            const step = currentTime => {
                ...
                if (progress < 1) requestAnimationFrame(step); else element.textContent = end.toLocaleString();
            };
            requestAnimationFrame(step);
        };
        if (isElementInViewport(element)) animate();
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const numberElements = document.querySelectorAll(".number-anim");
        numberElements.forEach((el => {
            const endValue = parseInt(el.getAttribute("data-number").replace(/\s/g, ""));
